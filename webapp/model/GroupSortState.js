sap.ui.define([
	...
	return BaseObject.extend("com.tender.agent.model.GroupSortState", {
		 ...
		 * @alias com.tender.agent.model.GroupSortState
		 */
		...
		/**
		 * Groups by TndrActp, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "TndrActp") {
				...
				this._oViewModel.setProperty("/sortBy", "TndrActp");

				aSorters.push(
					new Sorter("TndrActp", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Tknum");
			}
